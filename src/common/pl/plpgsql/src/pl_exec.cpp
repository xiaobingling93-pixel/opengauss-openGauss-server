...
static int exec_stmt_resignal(PLpgSQL_execstate* estate, PLpgSQL_stmt_signal* stmt);
...
static int expand_stmts(PLpgSQL_execstate* estate, List* stmts, bool& exception_flag, List* block_ptr_stack, bool resignal_in_handler = false);
static int expand_stmt_block(PLpgSQL_execstate* estate, PLpgSQL_stmt_block* stmt, bool& exception_flag, List* block_ptr_stack, bool resignal_in_handler = false);
static int expand_stmt_case(PLpgSQL_execstate* estate, PLpgSQL_stmt_case* stmt, bool& exception_flag, List* block_ptr_stack, bool resignal_in_handler = false);
...
static void pl_free_var(PLpgSQL_var* var);
...
/*
 * Check the autonomous procedure param
 * In distributed mode, ref cursor is not supported as param
 * In centralized mode, ref cursor is only supported as out param
 */
void CheckArgTypeMode(HeapTuple procTup, FunctionCallInfo fcinfo)
{
    bool argTypesIsNull = false;
    bool argModesIsNULL = false;
    bool prokindIsNULL = false;
    Datum proAllArgTypes = SysCacheGetAttr(PROCOID, procTup, Anum_pg_proc_proallargtypes, &argTypesIsNull);
#ifndef ENABLE_MULTIPLE_NODES
    Datum proArgModes = SysCacheGetAttr(PROCOID, procTup, Anum_pg_proc_proargmodes, &argModesIsNULL);
    ...
    ArrayType* arrAllArgTypes = DatumGetArrayTypeP(proAllArgTypes); /* ensure not toasted */
    int allNumArgs = ARR_DIMS(arrAllArgTypes)[0];
    Oid* argTypes = (Oid*)ARR_DATA_PTR(arrAllArgTypes);
#ifndef ENABLE_MULTIPLE_NODES
    ArrayType* arrArgModes = DatumGetArrayTypeP(proArgModes); /* ensure not toasted */
    char* argmodes = (char*)ARR_DATA_PTR(arrArgModes);
#endif
    for (int i = 0; i < allNumArgs; i++) {
#ifndef ENABLE_MULTIPLE_NODES
        /* ref cursor only supported as out param */
        if (argTypes[i] == REFCURSOROID && (argmodes[i] == PROARGMODE_IN || argmodes[i] == PROARGMODE_INOUT)) {
#else
        /* ref cursor not supported as param */
...
void SetInputParam(StringInfoData *buf, FunctionCallInfo fcinfo)
{
    for (int i = 0; i < fcinfo->nargs; i++) {
        if (i > 0)
            appendStringInfoChar(buf, ',');
...
/*
 * Scenarios where the input and output parameters of autonomous transactions are supported.
 */
static bool InOutParaIsSup(char prokind, Oid funcOid)
{
    if (PROC_IS_PRO(prokind) || (PROC_IS_FUNC(prokind) && is_function_with_plpgsql_language_and_outparam(funcOid))) {
        return true;
    }
    return false;
}

void SetInOutParam(HeapTuple procTup, StringInfoData *buf, FunctionCallInfo fcinfo)
{
    bool argTypesIsNull = false;
    bool argModesIsNULL = false;
    bool prokindIsNULL = false;
...
static char* AssembleAutomnousStatement(PLpgSQL_function* func, FunctionCallInfo fcinfo, char* sourceText)
{
    ...
    if (procedureOid != InvalidOid) {
        ...
        CheckArgTypeMode(procTup, fcinfo);

#ifndef ENABLE_MULTIPLE_NODES
        if (enable_out_param_override()) {
            SetInOutParam(procTup, &buf, fcinfo);
        } else {
#endif
            SetInputParam(&buf, fcinfo);
#ifndef ENABLE_MULTIPLE_NODES
...
typedef struct {
    /* reserved subxact's expr context */
    ExprContext* simple_econtext;

    /*
     * expr context's scope, no one refers it before this stackId.
     * '-1' means it can be released at any time.
     */
    int64 stackId;
...
/*
 * reserve current subxact's expr context into session list.
 */
void stp_reserve_subxact_exprcontext(SimpleEcontextStackEntry* simple_econtext, bool is_commit)
{
    MemoryContext oldcxt = MemoryContextSwitchTo(SESS_GET_MEM_CXT_GROUP(MEMORY_CONTEXT_STORAGE));
    XactExprContextItem *item = (XactExprContextItem*)palloc(sizeof(XactExprContextItem));
    ...
    while (item != NULL) {
        ...
        if (item->stackId > stackId  || item->stackId == -1) {
            ...
            if (preItem != NULL) {
                ...
                pfree(item);
                item = (XactExprContextItem*)preItem->next;
            } else {
                u_sess->plsql_cxt.spi_xact_expr_context = item->next;
                pfree(item);
                item = (XactExprContextItem*)u_sess->plsql_cxt.spi_xact_expr_context;
            }
        } else {
            preItem = item;
...
/*
 * release reserved subxact resource after 'stackId'.
 */
void stp_cleanup_subxact_resource(int64 stackId)
{
#ifndef ENABLE_MULTIPLE_NODES
    /* release reserved subxact expr context after 'stackId' */
    stp_cleanup_subxact_exprcontext(stackId);
#endif
...
void stp_reset_xact()
{
    /* cleanup all reserved subxact resource. */
...
void stp_reset_stmt()
{
    ...
    /* any reserved subxact resource in previous statement can be released now. */
    stp_cleanup_subxact_resource(-1);
}
...
void free_temp_func_tableof_index(List* temp_tableof_index)
{
    if (temp_tableof_index == NULL) {
        return;
    }
    ListCell* l = NULL;
    foreach (l, temp_tableof_index) {
        PLpgSQL_func_tableof_index* func_tableof = (PLpgSQL_func_tableof_index*)lfirst(l);
        hash_destroy(func_tableof->tableOfIndex);
    }

    list_free_deep(temp_tableof_index);
}


void free_func_tableof_index()
{
...
static TupleDesc get_cursor_tupledesc_exec(PLpgSQL_expr* expr, bool isOnlySelect, bool isOnlyParse)
{
    ...
    PG_CATCH();
    {
        ...
        expr->func->pre_parse_trig = temp_pre_parse_trig;
        ereport(ERROR, (errcode(ERRCODE_SYNTAX_ERROR),
                        errmsg("execute failed when parse the query: %s", expr->query),
...
static void exec_cursor_rowtype_init(PLpgSQL_execstate *estate, PLpgSQL_datum *datum, PLpgSQL_function *func)
{
    ...
    replaces = (bool *)palloc(new_natts * sizeof(bool));
    rc = memset_s(replaces, new_natts * sizeof(bool), true, new_natts * sizeof(bool));
    securec_check(rc, "\0", "\0");

    newvalues = (Datum *)palloc0(new_natts * sizeof(Datum));
...
static bool exception_matches_conditions(ErrorData* edata, PLpgSQL_condition* cond)
{
    ...
    for (; cond != NULL; cond = cond->next) {
        ...
        if (sqlerrstate == 0) {
        ...
        } else if (sqlstate != NULL && edata->sqlstate != NULL) {
            if (strcmp(sqlstate, edata->sqlstate) == 0) {
                return true;
            }
	         ...
	         */
            return true;
        } else if (condname != NULL) {
            if (strcmp(condname, "k_sqlwarning") == 0 && sqlerrstate == 1) {
                if (ERRCODE_TO_CATEGORY(edata->sqlerrcode) == ERRCODE_TO_CATEGORY(MAKE_SQLSTATE('0', '1', '0', '0', '0'))) {
                    return true;
                }
                if (edata->sqlstate != NULL && strlen(edata->sqlstate) == 5 && edata->sqlstate[0] == '0' && edata->sqlstate[1] == '1') {
                    return true;
                }
            } else if (strcmp(condname, "not_found") == 0 && sqlerrstate == 2) {
                if (ERRCODE_TO_CATEGORY(edata->sqlerrcode) == ERRCODE_TO_CATEGORY(MAKE_SQLSTATE('0', '2', '0', '0', '0'))) {
                    return true;
                }
                if (edata->sqlstate != NULL && strlen(edata->sqlstate) == 5 && edata->sqlstate[0] == '0' && edata->sqlstate[1] == '2') {
                    return true;
                }
            } else if (strcmp(condname, "k_sqlexception") == 0 && sqlerrstate == 3) {
                ...
                if (edata->sqlstate != NULL && strlen(edata->sqlstate) == 5 &&
                    !(edata->sqlstate[0] == '0' && edata->sqlstate[1] == '0') &&
                    !(edata->sqlstate[0] == '0' && edata->sqlstate[1] == '1') &&
                    !(edata->sqlstate[0] == '0' && edata->sqlstate[1] == '2')) {
                        return true;
                    }
            }
        }
...
static void exec_exception_end(PLpgSQL_execstate* estate, ExceptionContext *context)
{
    ...
    if (context->curExceptionCounter == u_sess->SPI_cxt.portal_stp_exception_counter &&
        ...
        stp_cleanup_subxact_resource(context->stackId);
    }
...
/*
 * A warning is generated, the warning is not caught in declare handler stmt.
 * A error is generated, the error can be caught in declare handler stmt.
 * A flag is added to indicate a declare handler stmt exists in a block.
 * If the flag is true, warning(class = '01') will be caught in declare handler stmt.
 */
static bool exec_is_declare_handler(PLpgSQL_exception* exception)
{
    ListCell *e = NULL;
    bool is_declare_handler = false;

    foreach (e, exception->action) {
        PLpgSQL_stmt *stmt = (PLpgSQL_stmt *)lfirst(e);
        if (stmt->cmd_type == PLPGSQL_STMT_BLOCK) {
            PLpgSQL_stmt_block *handler_block = (PLpgSQL_stmt_block *)stmt;
            
            if (handler_block->isDeclareHandlerStmt && handler_block->exceptions->exc_list != NULL) {
                is_declare_handler = true;
            }
        }
    }

    return is_declare_handler;
}
                                  ...
                                  ExceptionContext *context, int* coverage)
{
    ...
    foreach (e, block->exceptions->exc_list) {
        ...
        if (exception_matches_conditions(edata, exception->conditions)) {
            ...
            pl_free_var(state_var);
            state_var->value = (Datum)0;
            state_var->isnull = true;
            pl_free_var(errm_var);
...
static int exec_stmt_block(PLpgSQL_execstate* estate, PLpgSQL_stmt_block* block, List* block_ptr_stack, bool resignal_in_handler, int* coverage)
{
    ...
    for (i = 0; i < n_initvars; i++) {
        ...
        switch (estate->datums[n]->dtype) {
            ...
            case PLPGSQL_DTYPE_CURSORROW: {
                ...
                if (HeapTupleIsValid(rec->tup) && rec->default_val != NULL) {
    ...
    if (block->exceptions != NULL) {
        if (block->isTryCatch) {
            ...
            rc = exec_stmt_block_b_exception(estate, block, block_ptr_stack, resignal_in_handler);
 ...
 * ----------
 */
static int exec_stmt_block_b_exception(PLpgSQL_execstate* estate, PLpgSQL_stmt_block* block, List* block_ptr_stack, bool resignal_in_handler)
{
    estate->err_text = gettext_noop("during statement block entry");
    int rc = -1;
    ...
    rc = expand_stmts(estate, stmts, exception_flag, block_ptr_stack, resignal_in_handler);
    return rc;
}

static int expand_stmts(PLpgSQL_execstate* estate, List* stmts, bool& exception_flag, List* block_ptr_stack, bool resignal_in_handler)
{
    int rc = -1;
    int num_stmts = list_length(stmts);
    int stmtid = 0;
    ...
    while(stmtid < num_stmts && !early_exit && rc != PLPGSQL_RC_EXIT) {
        ...
        switch((enum PLpgSQL_stmt_types)stmt->cmd_type) {
            case PLPGSQL_STMT_BLOCK:
                rc = expand_stmt_block(estate, (PLpgSQL_stmt_block*)stmt, exception_flag, block_ptr_stack, resignal_in_handler);
                break;

            case PLPGSQL_STMT_IF:
                rc = expand_stmt_if(estate, (PLpgSQL_stmt_if*)stmt, exception_flag, block_ptr_stack, resignal_in_handler);
                break;

            case PLPGSQL_STMT_CASE:
                rc = expand_stmt_case(estate, (PLpgSQL_stmt_case*)stmt, exception_flag, block_ptr_stack, resignal_in_handler);
                break;

            case PLPGSQL_STMT_LOOP:
                rc = expand_stmt_loop(estate, (PLpgSQL_stmt_loop*)stmt, exception_flag, block_ptr_stack, resignal_in_handler);
                break;

            case PLPGSQL_STMT_WHILE:
                rc = expand_stmt_while(estate, (PLpgSQL_stmt_while*)stmt, exception_flag, block_ptr_stack, resignal_in_handler);
                break;
            ...
            default: {
                ...
                PG_TRY();
                {
                    ...
                    */
                    ...
                    estate->err_text = NULL;
                    // the block has declare handler stmt
                    if (block->exceptions->exc_list != NULL) {
                        estate->is_declare_handler = true;
                    }
...
static int expand_stmt_block(PLpgSQL_execstate* estate, PLpgSQL_stmt_block* stmt, bool& exception_flag, List* block_ptr_stack, bool resignal_in_handler)
{
    int rc = -1;
    ...
    rc = expand_stmts(estate, stmt->body, exception_flag, block_ptr_stack, resignal_in_handler);
    if (stmt->exceptions != NULL) {
...
static int expand_stmt_case(PLpgSQL_execstate* estate, PLpgSQL_stmt_case* stmt, bool& exception_flag, List* block_ptr_stack, bool resignal_in_handler)
{
    ...
    foreach (l, stmt->case_when_list) {
        ...
        if (!isnull && value) {
            ...
            return expand_stmts(estate, cwt->stmts, exception_flag, block_ptr_stack, resignal_in_handler);
        }
    }
    ...
    return expand_stmts(estate, stmt->else_stmts, exception_flag, block_ptr_stack, resignal_in_handler);
}

static int expand_stmt_if(PLpgSQL_execstate* estate, PLpgSQL_stmt_if* stmt, bool& exception_flag, List* block_ptr_stack, bool resignal_in_handler)
{
    bool value = false;
    bool isnull = false;
    ListCell* lc = NULL;
    ...
    exec_eval_cleanup(estate);
    if (!isnull && value) {
        return expand_stmts(estate, stmt->then_body, exception_flag, block_ptr_stack, resignal_in_handler);
    }
    ...
    foreach (lc, stmt->elsif_list) {
        ...
        exec_eval_cleanup(estate);
        if (!isnull && value) {
            return expand_stmts(estate, elif->stmts, exception_flag, block_ptr_stack, resignal_in_handler);
        }
    }

    return expand_stmts(estate, stmt->else_body, exception_flag, block_ptr_stack, resignal_in_handler);
}

static int expand_stmt_while(PLpgSQL_execstate* estate, PLpgSQL_stmt_while* stmt, bool& exception_flag, List* block_ptr_stack, bool resignal_in_handler)
{
...
static int expand_stmt_loop(PLpgSQL_execstate* estate, PLpgSQL_stmt_loop* stmt, bool& exception_flag, List* block_ptr_stack, bool resignal_in_handler)
{
    for(;;){
        int rc = expand_stmts(estate, stmt->body, exception_flag, block_ptr_stack, resignal_in_handler);
...
static int exec_stmts_savecursor(PLpgSQL_execstate* estate, List* stmts, int* coverage)
{
    ...
    PG_CATCH();
    {
        estate_cursor_set(t_thrd.log_cxt.call_stack);
        PG_RE_THROW();
    }
    PG_END_TRY();
...
static int exec_stmt(PLpgSQL_execstate* estate, PLpgSQL_stmt* stmt, bool resignal_in_handler, int* coverage)
{
    ...
    switch ((enum PLpgSQL_stmt_types)stmt->cmd_type) {
        ...
        case PLPGSQL_STMT_SIGNAL:
            rc = exec_stmt_signal(estate, (PLpgSQL_stmt_signal *)stmt);
            break;
        case PLPGSQL_STMT_RESIGNAL:
            if (!resignal_in_handler) {
                ereport(ERROR,
                    (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
...
static int exec_stmt_foreach_a(PLpgSQL_execstate* estate, PLpgSQL_stmt_foreach_a* stmt, bool resignal_in_handler, int* coverage)
{
    ...
    if (loop_var->dtype == PLPGSQL_DTYPE_REC ||
        ...
        loop_var->dtype == PLPGSQL_DTYPE_CURSORROW) {
        /*
...
/* ----------
 * exec_get_condition_information			get condition information item value for signal/resignal
 * ----------
 */
static void exec_get_condition_information(PLpgSQL_execstate* estate, PLpgSQL_stmt_signal* stmt,
    PLpgSQL_condition_info_item *con_item)
{
    ...
    foreach (lc, stmt->cond_info_item) {
        ...
        item_value = exec_eval_expr(estate, item->expr, &item_is_null, &item_typeid);
        if (item_is_null) {
            ereport(ERROR,
                (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                    errmsg("Variable '%s' can't be set to the value of 'NULL'", item->con_name)));
        }

        extval = convert_value_to_string(estate, item_value, item_typeid);

        switch (item->con_info_value) {
            case PLPGSQL_CLASS_ORIGIN:
                con_item->class_origin = pstrdup(extval);
                break;
            case PLPGSQL_SUBCLASS_ORIGIN:
                con_item->subclass_origin = pstrdup(extval);
                break;
            case PLPGSQL_MESSAGE_TEXT:
            ...
            case PLPGSQL_CONSTRAINT_CATALOG:
                ...
                break;
            case PLPGSQL_CONSTRAINT_SCHEMA:
                con_item->constraint_schema = pstrdup(extval);
                break;
            case PLPGSQL_CONSTRAINT_NAME:
                con_item->constraint_name = pstrdup(extval);
                break;
            case PLPGSQL_CATALOG_NAME:
                con_item->catalog_name = pstrdup(extval);
                break;
            case PLPGSQL_SCHEMA_NAME:
                con_item->schema_name = pstrdup(extval);
                break;
            case PLPGSQL_TABLE_NAME:
                con_item->table_name = pstrdup(extval);
                break;
            case PLPGSQL_COLUMN_NAME:
                con_item->column_name = pstrdup(extval);
                break;
            case PLPGSQL_CURSOR_NAME:
                con_item->cursor_name = pstrdup(extval);
                break;
            default:
                ereport(ERROR,
                        (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                            errmsg("unrecognized condition information item: %d in SIGNAL/RESIGNAL statement.", item->con_info_value)));
        }
...
static void StoreSignalError(int elevel, int code, PLpgSQL_condition_info_item *con_item, bool is_warning_throw, int is_signal)
{
    ereport(elevel, 
        (errcode(code),
            ...
            (con_item->subclass_origin != NULL) ? signal_subclass_origin(con_item->subclass_origin) : 0,
            (con_item->constraint_catalog != NULL) ? signal_constraint_catalog(con_item->constraint_catalog) : 0,
            (con_item->constraint_schema != NULL) ? signal_constraint_schema(con_item->constraint_schema) : 0,
            (con_item->constraint_name != NULL) ? signal_constraint_name(con_item->constraint_name) : 0,
            (con_item->catalog_name != NULL) ? signal_catalog_name(con_item->catalog_name) : 0,
            (con_item->schema_name != NULL) ? signal_schema_name(con_item->schema_name) : 0,
...
static void exec_free_con_item(PLpgSQL_condition_info_item *con_item)
{
    FREE_POINTER(con_item->sqlstate);
    FREE_POINTER(con_item->subclass_origin);
    FREE_POINTER(con_item->message_text);
    FREE_POINTER(con_item->constraint_catalog);
    FREE_POINTER(con_item->constraint_schema);
    FREE_POINTER(con_item->constraint_name);
    FREE_POINTER(con_item->catalog_name);
    FREE_POINTER(con_item->schema_name);
...
/* ----------
 * exec_stmt_signal			SIGNAL is the way to "retuen" an error.
 * ----------
 */
static int exec_stmt_signal(PLpgSQL_execstate* estate, PLpgSQL_stmt_signal* stmt)
{
    char *sqlstate = stmt->sqlstate;
    int elevel = ERROR;
...
/* ----------
 * exec_stmt_resignal		RESIGNAL makes it possible to both handle an error and return the error information.
 * ----------
 */
static int exec_stmt_resignal(PLpgSQL_execstate* estate, PLpgSQL_stmt_signal* stmt)
{
    ErrorData *cur_errdata = estate->cur_error;
    char *sqlstate = stmt->sqlstate;
...
static int exec_stmt_open(PLpgSQL_execstate* estate, PLpgSQL_stmt_open* stmt)
{
    ...
    if (curvar->ispkg && u_sess->is_autonomous_session) {
        ereport(ERROR,
            (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
            errmodule(MOD_PLSQL),
            errmsg("package cursor referenced in autonomous procedure is not supported yet"),
            errdetail("cursor \"%s\" in package \"%s\" referenced in autonomous procedure is not supported yet",
                curvar->refname, curvar->pkg->pkg_signature)));
    }
#endif
    if (!curvar->isnull) {
        if (curvar->pkg != NULL) {
...
static int exec_stmt_fetch(PLpgSQL_execstate* estate, PLpgSQL_stmt_fetch* stmt)
{
...
#ifndef ENABLE_MULTIPLE_NODES
    /* in centralized mode, package cursor used in autonomous session is not supported */
    bool isPkgCurInAutoSess = curvar->ispkg && u_sess->is_autonomous_session;
    if (isPkgCurInAutoSess) {
        ereport(ERROR,
            (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
            errmodule(MOD_PLSQL),
            errmsg("package cursor referenced in autonomous procedure is not supported yet"),
    ...
    if (portal->have_rollback_transaction) {
        ereport(ERROR,
            (errcode(ERRCODE_NULL_VALUE_NOT_ALLOWED),
                errmodule(MOD_PLSQL),
                errmsg("The data has been rollback, and the cursor data cannot be obtained.")));
    }
    ...
    if (!stmt->is_move) {
        ...
        if (n != 0) {
            if (stmt->bulk_collect) {
                ...
                if (rec) {
                    target = (PLpgSQL_datum*)rec;
                } else {
                    target = (PLpgSQL_datum*)row;
                }

                if (target->dtype != PLPGSQL_DTYPE_CURSORROW) {
                    exec_move_row(estate, rec, row, tuptab->vals[0], tuptab->tupdesc);
                } else {
                    exec_move_row_from_fields(estate, target, tuptab->vals[0], tuptab->tupdesc);
                }
            }
        } else {
            if (stmt->bulk_collect) {
    ...
    if (B_FETCH && n == 0) {
        ereport(ERROR,
                (errcode(ERRCODE_NO_DATA),
                    errmodule(MOD_PLSQL),
                    errmsg("No data - zero rows fetched, selected, or processed")));
    }
...
static int exec_stmt_transaction(PLpgSQL_execstate *estate, PLpgSQL_stmt* stmt)
{
    ...
    if(strcmp(PORTAL, ResourceOwnerGetName(t_thrd.utils_cxt.CurrentResourceOwner)) == 0) {
        ResourceOwner nextChild = ResourceOwnerGetNextChild(t_thrd.utils_cxt.CurrentResourceOwner);
        if(nextChild && ResourceOwnerIsValid(nextChild) && strcmp(PORTAL, ResourceOwnerGetName(nextChild)) == 0) {
            ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                errmsg("commit with Cursor portal execute is not supported"),
                    errdetail("Execute PL/pgSQL function(%s).", estate->func->fn_signature),
    ...
    // 2. Hold portals.
    needResetErrMsg = stp_disable_xact_and_set_err_msg(&savedisAllowCommitRollback, STP_XACT_OPEN_FOR);
    _SPI_hold_cursor(((PLpgSQL_stmt_types)stmt->cmd_type) == PLPGSQL_STMT_ROLLBACK);
    stp_reset_xact_state_and_err_msg(savedisAllowCommitRollback, needResetErrMsg);
    if (t_thrd.utils_cxt.STPSavedResourceOwner == NULL &&
        u_sess->SPI_cxt.portal_stp_exception_counter == 0 &&
        u_sess->plsql_cxt.stp_savepoint_cnt == 0) {
...
void exec_assign_expr(PLpgSQL_execstate* estate, PLpgSQL_datum* target, PLpgSQL_expr* expr)
{
    ...
    ExecTableOfIndexInfo TableOfIndexInfo;
    errno_t rc = memset_s(&TableOfIndexInfo, sizeof(ExecTableOfIndexInfo), 0, sizeof(ExecTableOfIndexInfo));
    securec_check(rc, "\0", "\0");

    value = exec_eval_expr(estate, expr, &isnull, &valtype, &tableOfIndex, &TableOfIndexInfo);
...
static void exec_move_row_from_fields(PLpgSQL_execstate *estate, PLpgSQL_datum *target, HeapTuple var_tup, TupleDesc var_tupdesc)
{
    int fnum = 0;
    int anum = 0;
    int rc = 0;
    ...
    replaces = (bool *)palloc(td_natts * sizeof(bool));
    rc = memset_s(replaces, td_natts * sizeof(bool), true, td_natts * sizeof(bool));
    securec_check(rc, "\0", "\0");
    ...
    if (var_tupdesc != tupdesc) {
        ...
        newtup = (HeapTuple)tableam_tops_modify_tuple(rec->tup, rec->tupdesc, newvalues, newnulls, replaces);
    }
...
void exec_assign_value(PLpgSQL_execstate* estate, PLpgSQL_datum* target, Datum value, Oid valtype,
    bool* isNull, HTAB* tableOfIndex, ExecTableOfIndexInfo* tableOfIndexInfo)
{
    switch (target->dtype) {
        case PLPGSQL_DTYPE_VARRAY:
        case PLPGSQL_DTYPE_TABLE:
...
#endif
                tmptup.t_data = td;
                exec_move_row_from_fields(estate, target, &tmptup, tupdesc);
                ReleaseTupleDesc(tupdesc);
            }
        } break;
                      ...
                      int nsubscripts, int pos, int* subscriptvals, Oid subscriptType, HTAB* tableOfIndex)
{
    ...
    if (subscriptType == VARCHAROID) {
        ...
        exprdatum = Int32GetDatum(exec_eval_integer(estate, subscripts[nsubscripts - 1 - pos], &subisnull));
    }
    if (subisnull) {
        ereport(ERROR,
            (errcode(ERRCODE_NULL_VALUE_NOT_ALLOWED),
...
void exec_get_datum_type_info(PLpgSQL_execstate* estate, PLpgSQL_datum* datum, Oid* typeId, int32* typmod,
    Oid* collation, List** tableOfIndexType, PLpgSQL_function* func)
{
    switch (datum->dtype) {
        case PLPGSQL_DTYPE_VARRAY:
        case PLPGSQL_DTYPE_TABLE:
        case PLPGSQL_DTYPE_VAR: {
            ...
            if (var->nest_table != NULL) {
                ...
                *tableOfIndexType = lappend_oid(*tableOfIndexType, var->tableOfIndexType);
                while (nest != NULL) {
                    *typeId = nest->datatype->typoid;
                    *typmod = nest->datatype->atttypmod;
                    *tableOfIndexType = lappend_oid(*tableOfIndexType, nest->tableOfIndexType);
    ...
    HTAB** tableOfIndex, ExecTableOfIndexInfo* tableOfIndexInfo)
{
     ...
     * directly
     */
    if (exec_eval_simple_expr(estate, expr, &result, isNull, rettype, tableOfIndex, tableOfIndexInfo)) {
#ifdef ENABLE_MULTIPLE_NODES
        stp_reset_xact_state_and_err_msg(savedisAllowCommitRollback, needResetErrMsg);
#else
        stp_check_transaction_and_create_econtext(estate, oldTransactionId);
    ...
    HTAB** tableOfIndex, ExecTableOfIndexInfo* tableOfIndexInfo)
{
    ...
    *result = ExecEvalExpr(expr->expr_simple_state, econtext, isNull);
    ...
    if (plpgsql_estate && plpgsql_estate->curr_nested_table_type != InvalidOid) {
        ...
        if (plpgsql_estate->curr_nested_table_layers != 0 && tableOfIndexInfo!= NULL) {
            tableOfIndexInfo->isnestedtable = true;
            tableOfIndexInfo->tableOfLayers = plpgsql_estate->curr_nested_table_layers;
            plpgsql_estate->curr_nested_table_layers = 0;
        }
        plpgsql_estate->curr_nested_table_type = InvalidOid;
    }
#ifndef ENABLE_MULTIPLE_NODES
 ...
 */
static TupleConversionMap *convert_tuples_for_bulk_collect(TupleDesc indesc, TupleDesc outdesc, const char *msg)
{
...
/*
 * For tuple with only ONE attribute, if the attribute with record type or just same with
 * request type, we regard the only one attribute is for the request type, and return the
 * attribute num. Return -1 for other cases.
 */
static int get_bulk_collect_record_attnum(TupleDesc tupdesc, HeapTuple tup, Oid typeoid)
{
    int last_anum = 0;
    int cnt = 0;
    int td_natts = tupdesc ? tupdesc->natts : 0;
 ...
 */
static Datum exec_tuple_get_composite(PLpgSQL_execstate* estate, TupleDesc tupdesc, HeapTuple tup,
    Oid typeoid, int last_anum, TupleConversionMap* converMap)
{
    Assert(tupdesc != NULL);
    Assert(last_anum != -1 || converMap != NULL);
    ...
    /* last_num != -1 means the only ONE attribute is for the request type */
    if (last_anum != -1) {
        tuptype = SPI_gettypeid(tupdesc, last_anum + 1);
    }
    if (tuptype == RECORDOID || tuptype == typeoid) {
        resetStringInfo(&buf);
    ...
    /* Other cases, transform tuple into typeoid */
    TupleDesc outTupdesc = converMap->outdesc;
    AttrNumber *attrMap = converMap->attrMap;
    Datum *invalues = converMap->invalues;
    bool *inisnull = converMap->inisnull;
    Datum *outvalues = converMap->outvalues;
    bool *outisnull = converMap->outisnull;
    int outnatts = converMap->outdesc->natts;
    int i;
    /*
     ...
     */
    heap_deform_tuple(tup, converMap->indesc, invalues + 1, inisnull + 1);
    ...
    for (i = 0; i < outnatts; i++) {
        int j = attrMap[i];
        if (j == 0) {
            /* means dropped attribute, just set null */
    ...
    HeapTuple result = heap_form_tuple(outTupdesc, outvalues, outisnull);
    return HeapTupleGetDatum(result);
}
...
static void exec_assign_bulk_collect(PLpgSQL_execstate *estate, bulk_collect_context context)
{
    ...
    if (context.var->datatype->collectionType == PLPGSQL_COLLECTION_TABLE &&
        ...
        if (context.ntup != 0) {
            ...
            newTableOfIndex = createTableOfIndex();
        }
    }
...
static void exec_read_bulk_collect(PLpgSQL_execstate* estate, PLpgSQL_row* row, SPITupleTable* tuptab)
{
    ...
    for (fnum = 0; fnum < row->nfields; fnum++) {
        ...
        if (context.ntup == 0) {
            ...
            int tupLastAnum = -1;
            TupleDesc outTupdesc = NULL;
            TupleConversionMap* converMap = NULL;
            if (need_entire_row) {
                /* check if the tuple contain only one attribute, and is for the request type */
                tupLastAnum = get_bulk_collect_record_attnum(context.tupdesc, tuptab->vals[0], context.elemtype);
                if (tupLastAnum == -1) {
                    /* not the situation above, we need to build convert map */
                    outTupdesc = lookup_rowtype_tupdesc(context.elemtype, -1);
                    converMap = convert_tuples_for_bulk_collect(context.tupdesc, outTupdesc,
                        gettext_noop("bulk collect returned record type does not match expected record type"));
                }
            }
            for (int i = 0; i < context.ntup; i++) {
                int t_natts = 0;
                ...
                if (need_entire_row) {
                    context.valtype = context.elemtype;
                    context.values[i] = exec_tuple_get_composite(estate, context.tupdesc, tup,
                        context.elemtype, tupLastAnum, converMap);
                ...
                bool forCursor = context.isnull[i] == false && CheckTypeIsCursor(row, context.valtype, fnum) &&
                    estate->cursor_return_data != NULL;
                if (forCursor) {
    ...
    if (valtype != reqtype || reqtypmod != -1) {
        ...
        if (!isnull) {
            ...
            if (funcid != InvalidOid && !(result == COERCION_PATH_COERCEVIAIO || result == COERCION_PATH_ARRAYCOERCE)) {
                if (reqinput != NULL && F_SUBTYPE_IN == reqinput->fn_oid) {
                    /* SUBTYPE XXX IS NUMBER(8,2);
                     * When use sql shown above we should use subtype_in
                     * instead of original process.
                     */
...
#ifndef ENABLE_MULTIPLE_NODES
        /* reserve it into session, and free it latter */
        stp_reserve_subxact_exprcontext(u_sess->plsql_cxt.simple_econtext_stack, (event == SUBXACT_EVENT_COMMIT_SUB));
#else
...
/*
 * pl_free_var --- pfree any pass-by-reference value of the variable.
 *
 ...
 */
#ifndef ENABLE_MULTIPLE_NODES
static void CheckAutoRefCursor(PLpgSQL_execstate* estate, int dno)
{
    /* only conside autonomous transaction procedure */
    if (u_sess->is_autonomous_session != true || u_sess->SPI_cxt._connected != 0) {
        return;
...
static void CheckNestedTableofVar(PLpgSQL_execstate* estate, int dno)
{
    /* only conside autonomous transaction procedure */
    ...
    PLpgSQL_datum* target = estate->datums[dno];
    if (target->dtype != PLPGSQL_DTYPE_VAR && target->dtype != PLPGSQL_DTYPE_ASSIGNLIST) {
        return;
    }

    if (target->dtype == PLPGSQL_DTYPE_ASSIGNLIST) {
        PLpgSQL_assignlist* assignvar = (PLpgSQL_assignlist*)target;
        target = estate->datums[assignvar->targetno];
        if (target->dtype != PLPGSQL_DTYPE_VAR) {
            return;
        }
    }
...
/*
 * check the assign target if valid
 * first assign target can not be
 * autonomous procedure out ref_cursor arg,
 * second, assign target can not be nested table of var in automous session
 */
static void CheckAssignTarget(PLpgSQL_execstate* estate, int dno) {
    if (likely(!u_sess->is_autonomous_session)) {
        return;
    }
